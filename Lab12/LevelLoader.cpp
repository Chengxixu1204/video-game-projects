...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		else if (type == "Player")
		{
            ...
            Vector3 pos;
            Arrow* aw = new Arrow(game, parent);
            ...
            Checkpoint* ck = new Checkpoint(game, parent);
            std::string temp;
            std::string ss;
            if (GetStringFromJSON(actorValue, "level", temp))
            {
                ck->setlevel(temp);
            }
            if (GetStringFromJSON(actorValue, "text", ss))
            {
                ck->settext(ss);
            }
            if(game->allcheck.size() == 0){
                ck->setactive(true);
            }else{
                ck->setactive(false);
            }
            game->allcheck.push(ck);
            actor = ck;
        }else if(type == "LaserMine"){
            LaserMine* lm = new LaserMine(game, parent);
            actor = lm;
        }else if(type == "Coin"){
            Coin* co = new Coin(game, parent);
            actor = co;
        }else if(type == "SecurityCamera"){
            SecurityCamera* sc = new SecurityCamera(game, parent);
            float inter;
            if (GetFloatFromJSON(actorValue, "interpTime", inter))
            {
                sc->interpTime = inter;
            }
            float pa;
            if (GetFloatFromJSON(actorValue, "pauseTime", pa))
            {
                sc->pauseTime = pa;
                sc->pauseTime2 = pa;
            }
            Quaternion st;
            if (GetQuaternionFromJSON(actorValue, "startQ", st))
            {
                sc->startQ = st;
            }
            Quaternion en;
            if (GetQuaternionFromJSON(actorValue, "endQ", en))
            {
                sc->endQ = en;
            }
            actor = sc;
