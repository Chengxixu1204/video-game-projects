...
void LaserComponent::Update(float deltaTime){
    ...
    vls.push_back(ls);
    CastInfo ci;
    bool temp1 = SegmentCast(reinterpret_cast<Actor*>(mOwner->GetGame()->pl), ls, ci);
    if(temp1){
        vls.back().mEnd = ci.mPoint;
        Mix_PlayChannel(-1, mOwner->GetGame()->GetSound("Assets/Sounds/Laser.wav"), 0);
        mOwner->GetGame()->pl->requestspawn();
    }else{
        CastInfo ai;
        bool temp2 = SegmentCast(mOwner->GetGame()->bvec, ls, ai, mOwner->mParent);
        while(temp2 && reinterpret_cast <Block*>(ai.mActor)->Getmirror()){
            ...
            vls.push_back(ls2);
            temp2 = SegmentCast(mOwner->GetGame()->bvec, ls2, ai, ai.mActor);
        }
    }
}
