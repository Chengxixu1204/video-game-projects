...
:MoveComponent(owner)
{
    ...
    run_chanel = Mix_PlayChannel(-1, mOwner->GetGame()->GetSound("Assets/Sounds/Running.wav"), -1);
    Mix_Pause(run_chanel);
}
...
void PlayerMove::Update(float deltaTime){
    ...
    if((mCurrentState == MoveState::OnGround && mVelocity.Length() > 50.0f) || (mCurrentState == MoveState::WallRun) || (mCurrentState == MoveState::WallClimb)){
        Mix_Resume(run_chanel);
    }else{
        Mix_Pause(run_chanel);
    }
}

void PlayerMove::getrespawn(){
    mOwner->SetPosition(mOwner->GetGame()->pl->respawn);
...
bool PlayerMove::CanWallClimb(CollSide cs){
    ...
    if(cs == CollSide::Right){
        if(Vector3::Dot(mOwner->GetForward(), Vector3(0, 1, 0)) < -0.9){
            if((Vector2::Dot(Vector2::Normalize(Vector2(mVelocity.x, mVelocity.y)), Vector2(0, 1)) < -0.9) && (Vector2(mVelocity.x, mVelocity.y).Length() > 350)) {
                result = true;
            }
        }
        ...
        if(Vector3::Dot(mOwner->GetForward(), Vector3(1, 0, 0)) < -0.9){
            if((Vector2::Dot(Vector2::Normalize(Vector2(mVelocity.x, mVelocity.y)), Vector2(1, 0)) < -0.9) && (Vector2(mVelocity.x, mVelocity.y).Length() > 350)) {
                result = true;
            }
        }
    }else if(cs == CollSide::Back){
        if(Vector3::Dot(mOwner->GetForward(), Vector3(-1, 0, 0)) < -0.9){
            if((Vector2::Dot(Vector2::Normalize(Vector2(mVelocity.x, mVelocity.y)), Vector2(-1, 0)) < -0.9) && (Vector2(mVelocity.x, mVelocity.y).Length() > 350)) {
                result = true;
            }
...
bool PlayerMove::CanWallRun(CollSide cs){
    bool result = false;
    if(cs == CollSide::Right){
        if(Vector3::Dot(mOwner->GetForward(), Vector3(0, 1, 0)) < 0.6){
            if((Vector2::Dot(Vector2::Normalize(Vector2(mVelocity.x, mVelocity.y)), Vector2(mOwner->GetForward().x, mOwner->GetForward().y)) > 0) && (Vector2(mVelocity.x, mVelocity.y).Length() > 350)) {
                result = true;
                Runside = Vector3(0, 1, 0);
            }
        }
    }else if(cs == CollSide::Left){
        if(Vector3::Dot(mOwner->GetForward(), Vector3(0, -1, 0)) < 0.6){
        ...
        if(Vector3::Dot(mOwner->GetForward(), Vector3(1, 0, 0)) < 0.6){
            if((Vector2::Dot(Vector2::Normalize(Vector2(mVelocity.x, mVelocity.y)), Vector2(mOwner->GetForward().x, mOwner->GetForward().y)) > 0) && (Vector2(mVelocity.x, mVelocity.y).Length() > 350)) {
                result = true;
                Runside = Vector3(1, 0, 0);
            }
        }
    }else if(cs == CollSide::Back){
        if(Vector3::Dot(mOwner->GetForward(), Vector3(-1, 0, 0)) < 0.6){
