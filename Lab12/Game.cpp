...
void Game::RemoveBlock(Block* b){
    std::vector<Actor*>::iterator position = std::find(bvec.begin(), bvec.end(), b);
    if (position != bvec.end())
        bvec.erase(position);
}

void Game::AddCam(SecurityCamera* sc){
    allcam.push_back(sc);
}


void Game::RemoveCam(SecurityCamera* sc){
    std::vector<SecurityCamera*>::iterator position = std::find(allcam.begin(), allcam.end(), sc);
    if (position != allcam.end())
        allcam.erase(position);
}
...
void Game::LoadData()
{
    ...
    Matrix4 view = Matrix4::CreateLookAt(Vector3(0, 0, 0), Vector3(1, 0, 0), Vector3::UnitZ);
