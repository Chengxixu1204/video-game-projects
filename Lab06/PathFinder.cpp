...
bool PathFinder::CalculatePath(class PathNode* start, class PathNode* end, std::vector<class PathNode*>& outPath)
{
    ...
    std::map<PathNode*, NodeInfo> info;
    PathNode* currentnode = start;
    info[currentnode].IsClosed = true;
    do{
        for(size_t i = 0; i <currentnode->mAdjacent.size(); i++){
            PathNode* n = currentnode->mAdjacent[i];
            if(!info[n].IsClosed){
                if(openset.find(n) != openset.end()){
                    float newg = info[n].g + Vector2::Distance(currentnode->GetPosition(), n->GetPosition());
                    if(newg < info[n].g){
                    ...
                    info[n].parent = currentnode;
                    info[n].h = Vector2::Distance(currentnode->GetPosition(), end->GetPosition());
    ...
    while(currentnode != start){
        outPath.push_back(info[currentnode].parent);
