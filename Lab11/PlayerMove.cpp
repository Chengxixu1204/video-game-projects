...
void PlayerMove::getrespawn(){
    mOwner->SetPosition(mOwner->GetGame()->Getpl()->respawn);
...
void PlayerMove::UpdateOnGround(float deltaTime){
    ...
    for(size_t i = 0; i < mOwner->GetGame()->getallblocks().size(); i++){
        CollSide result = FixCollision(mOwner->GetComponent<CollisionComponent>(), mOwner->GetGame()->getallblocks()[i]->GetComponent<CollisionComponent>());
...
void PlayerMove::UpdateWallRun(float deltaTime){
    ...
    for(size_t i = 0; i < mOwner->GetGame()->getallblocks().size(); i++){
        CollSide result;
        result = FixCollision(mOwner->GetComponent<CollisionComponent>(), mOwner->GetGame()->getallblocks()[i]->GetComponent<CollisionComponent>());
    }
    if(mVelocity.z <= 0){
        mVelocity.z = 0.0f;
        ChangeState(MoveState::Falling);
